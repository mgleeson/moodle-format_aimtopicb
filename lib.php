...
/**
 * This file contains main class for AIMtopicb course format.
 *
 * @since     Moodle 2.0
 ...
 */
const FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT = -1;

/** @var int Deepest level of page to let users create.  Must be between the root level and the topic level. */
const FORMAT_AIMTOPICB_SECTION_LEVEL_PAGE_USE = 1;

/** @var int Level of topics, which are displayed within pages. */
const FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC = 2;
// END ADDED.

/**
 * Main class for the AIMtopicb course format.
 *
 * @package   format_aimtopicb
 * @copyright 2019 onwards James Calder and Otago Polytechnic
 * @copyright based on work by 2009 Sam Hemelryk
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_aimtopicb extends core_courseformat\base {

    // ADDED.
    /** @var int ID of section 0 / the General section, treated as the section root by the AIMtopicb format */
    public $fmtrootsectionid;

    /** @var stdClass AIMtopicb-specific section information */
    private $fmtsections = null;

    /** @var course_modinfo|null Mod info the AIMtopicb-specific section information was based on */
    private $fmtmodinfo = null;

    /** @var bool AIMtopicb-specific section information is complete*/
    ...
    public function page_title(): string {
        return get_string_manager()->string_exists('sectionoutline', 'format_aimtopicb') ?
                get_string('sectionoutline', 'format_aimtopicb') : get_string('topicoutline');
    }
    ...
    final public function fmt_get_sections($needall = true) : array {
        ...
        if (isset($this->fmtsections) && $this->fmtmodinfo == $modinfo) {
            ...
            $courseperioddays = format_aimtopicb_duration_as_days($course->periodduration);
            ...
            $sectionprevatlevel = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,
                                            FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1, null);

            // The current section at, or above, each level.
            $sectionatlevel = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,
                                        FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1, null);
            ...
            foreach ($sections as $thissection) {
                ...
                $levelsan = ($sectionatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT] == null) ?
                        FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT
                        : max(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1,
                        min($thissection->level ?? FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC, FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC));
                $thissection->levelsan = $levelsan;

                // Update remembered sections.
                for ($sublevel = $levelsan; $sublevel <= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC; $sublevel++) {
                ...
                // The previous page.
                $thissection->prevpageid = $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - 1] ?
                                                $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - 1]->id
                                                : null;

                // The previous section at any level.
                $thissection->prevanyid = $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC] ?
                                                $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC]->id
                                                : null;

                // The section's parent.
                $thissection->parentid = ($levelsan > FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT) ?
                ...
                $thissection->parentvisiblesan = ($levelsan <= FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT) ?
                                                    true
                                                    : $sectionatlevel[$levelsan - 1]->visiblesan;
                $thissection->visiblesan = ($levelsan <= FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT) ?
                                                    true
                                                    : ($sectionatlevel[$levelsan - 1]->visiblesan && $thissection->visible);

                // Set date-start property from previous section.
                $thissection->datestart = $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC] ?
                                                $sectionprevatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC]->dateend
                                                : (is_null($courseperioddays) ? null : $course->startdate);

                // Set date-end property.
                if ($levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) {
                    $sectionperioddays = 0;
                } else {
                    $sectionperioddays = format_aimtopicb_duration_as_days($thissection->periodduration);
                ...
                $thissection->currentnestedlevel = $iscurrent ? FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC
                                                            : FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT - 1;
        ...
        if ($needall && !$this->fmtsectionscomplete) {
            ...
            // Remembered sections.
            $sectionnextatlevel = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,
                                            FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1, null);
            ...
            for ($thissection = end($fmtsections); /* ... */
                ...
                $thissection->nextupid = $sectionnextatlevel[$levelsan] ?
                                                ...
                                                : null;
                $thissection->nextpageid = $sectionnextatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - 1] ?
                                                $sectionnextatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - 1]->id
                                                : null;
                $thissection->nextanyid = $sectionnextatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC] ?
                                                $sectionnextatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC]->id
                                                : null;
                ...
                if ($thissection->parentid) {
                    ...
                    if ($thissection->currentnestedlevel >= FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT) {
    ...
    public function get_section_name($section) : string {
        ...
        $weeksword = get_string_manager()->string_exists('weeks_capitalised', 'format_aimtopicb') ?
            get_string('weeks_capitalised', 'format_aimtopicb') : get_string('weeks');
        // Figure out the string for the week number.
        $daystring = '';
        if ($section->dateend && ($section->datestart < $section->dateend)) {
            $currentyear = format_aimtopicb_week_date(time())->o;
            $datestart = format_aimtopicb_week_date($section->datestart + 12 * 60 * 60);
            $dateend = format_aimtopicb_week_date($section->dateend - 12 * 60 * 60);
    ...
    /**
     * Returns the default section name for the AIMtopicb course format.
     *
    ...
    public function get_default_section_name($section) : string {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_aimtopicb');
        } else {
    ...
    public function set_section_number($singlesection): void {
        ...
        if (isset($singlesection) && $singlesection->level >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) {
            $sections = $this->fmt_get_sections();
            $singlesection = $sections[$singlesection->id];
            if (isset($singlesection) && $singlesection->levelsan >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) {
    ...
    protected function fmt_set_get_sections_preferences(string $preferencename = null, array $sectionids = null): array {
        ...
        foreach ($sections as $sectionid => $section) {
            if ($sectionid > $collapsedsetold
                    && $section->levelsan >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC
    ...
    public function get_view_url($section, $options = []) {
        global $CFG;
        $course = $this->get_course();
        $url = new moodle_url( ($options['fmtedit'] ?? false) ? '/course/format/aimtopicb/_course_view.php'
        ...
        if ($section !== null) {                                                // CHANGED.
            ...
            // CHANGED.
            $pageid = ($section->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) ? $section->id : $section->parentid;
            if ($pageid != $this->fmtrootsectionid) {
                $url->param('sectionid', $pageid);
            }
            if ($section->levelsan >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) {
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {
        ...
        $navigationwrapper = new \format_aimtopicb\global_navigation_wrapper($navigation); // ADDED.
     ...
     *
     * AIMtopicb format uses the following options:
    ...
    public function course_format_options($foreditform = false) : array {
        ...
        if ($foreditform && !isset($courseformatoptions['hiddensections']['label'])) { // CHANGED.
            $courseformatoptionsedit = [
                ...
                'periodduration' => [
                    'label' => new lang_string('perioddurationdefault', 'format_aimtopicb'), // CHANGED.
                    'help' => 'perioddurationdefault',
                    'help_component' => 'format_aimtopicb',                    // CHANGED.
                    'element_type' => 'select',                                 // CHANGED.
                    // REMOVED: Replaced periodduration type.
                    // ADDED.
                    'element_attributes' => [[
                        null => new lang_string('period_undefined', 'format_aimtopicb'),
                ...
                // ADDED.
                'collapsible' => [
                    'label' => get_string('collapsibledefault', 'format_aimtopicb'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            '0' => get_string('no'),
                    ...
                    'help_component' => 'format_aimtopicb',
                ],
                // END ADDED.
                'hiddensections' => [
                ...
                // ADDED.
                'bannerslice' => [
                    'label' => new lang_string('bannerslice', 'format_aimtopicb'),
                    'help' => 'bannerslice',
                    'help_component' => 'format_aimtopicb',
                    'element_type' => 'select',
                    'element_attributes' => [
    ...
    public function section_format_options($foreditform = false) : array {
        ...
        if ($sectionformatoptions === false) {
            $sectionformatoptions = [
                ...
                'level' => [
                    'default' => FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC,         // CHANGED.
                    'type' => PARAM_INT
                ],
                // END INCLUDED.
        ...
        if ($foreditform && !isset($sectionformatoptions['level']['label'])) {
            $sectionformatoptionsedit = [
                ...
                'level' => [
                    ...
                    'label' => get_string('level', 'format_aimtopicb'),        // CHANGED.
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1 => get_string('asprincipal', 'format_aimtopicb'), // CHANGED.
                            FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 2 => get_string('aschild', 'format_aimtopicb'),     // CHANGED.
                            FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC => get_string('topic') // ADDED.
                        ]
                    ],
                    'help' => 'level',
                    'help_component' => 'format_aimtopicb',
                ...
                'periodduration' => [
                    'label' => new lang_string('perioddurationoverride', 'format_aimtopicb'), // CHANGED.
                    ...
                    'element_attributes' => [[
                        ...
                        '0 day' => new lang_string('period_0_days', 'format_aimtopicb'),
                        '1 day' => new lang_string('numday', '', 1),
                ...
                // ADDED.
                'collapsible' => [
                    'label' => get_string('collapsibleoverride', 'format_aimtopicb'),
                    'element_type' => 'select',
                    'element_attributes' => [
                        [
                            null => new lang_string('default'),
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) : array {
        ...
        if (!$forsection) {
            $PAGE->requires->js('/course/format/aimtopicb/_course_edit.js');
        }
        // END ADDED.
     ...
     *
     * If the course format was changed to 'aimtopicb', we try to copy options
    ...
    public function course_header() {
        ...
        // ADDED.
        return new \format_aimtopicb\courseheader($this->get_course());
        // END ADDED.
    }
    // END INCLUDED.
    ...
    public function course_content_header() {
        ...
        if (class_exists('\format_aimtopicb\output\renderer')) {
            return new \format_aimtopicb\coursecontentheaderfooter($PAGE, -1);
        } else {
            return null;
        }
    ...
    public function is_section_current($section) : bool {
        ...
        return ($section->section && $section->currentnestedlevel >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC); // CHANGED.
    }
    // END INCLUDED.
    ...
    public function section_action($section, $action, $sr): array {
        ...
        $renderer = $PAGE->get_renderer('format_aimtopicb');                   // CHANGED.
 ...
 */
function format_aimtopicb_inplace_editable(string $itemtype, int $itemid, $newvalue) : \core\output\inplace_editable {
    // CHANGED LINE ABOVE.
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            ...
            [$itemid, 'aimtopicb'], MUST_EXIST);                               // CHANGED.
