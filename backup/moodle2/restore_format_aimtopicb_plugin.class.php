...
/**
 * Specialised restore for AIMtopicb course format.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_format_aimtopicb_plugin extends restore_format_plugin {
    ...
    public function after_restore_course() {
        ...
        if ($backupinfo->original_course_format !== 'aimtopicb') {
        ...
        $DB->execute(
            "UPDATE {course_format_options} SET value = '0 day'
             WHERE courseid = ? AND format = 'aimtopicb' AND name = 'periodduration' AND value = '0 days'",
        ...
        $DB->execute(
            "INSERT INTO {course_format_options} (courseid, format, sectionid, name, value)
             ...
             WHERE courseid = ? AND format = 'aimtopicb' AND name = 'periodduration' AND value LIKE '0 %' AND NOT EXISTS (
                SELECT id
