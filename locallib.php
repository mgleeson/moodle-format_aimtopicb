 ...
 */
function format_aimtopicb_set_section_visible(int $courseid, \stdClass $section, int $visibility) : array {
 ...
 */
function format_aimtopicb_course_create_section(\stdClass $courseorid, \stdClass $section) : \stdClass {
    ...
    if (true) {                                                                 // CHANGED: We've already checked the parent exists.
        ...
        format_aimtopicb_move_section_to($course, $cw, $section, true);        // CHANGED: Use section info instead of position.
        // END CHANGED.
        ...
        $cw->levelsan = $section->level ?? FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC; // ADDED.
    }
 ...
 */
function format_aimtopicb_move_section_to(\stdClass $course, $origins, \stdClass $destination) {
    ...
    $movedsections = format_aimtopicb_reorder_sections($sections, $origins, $destination); // CHANGED.
    ...
    foreach ($movedsections as $id => $movedsection) {
        ...
        if ($movedsection->level < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC && $sections[$id]->periodduration != '0 day') {
 ...
 * @return bool
 */
function format_aimtopicb_course_can_delete_section(\stdClass $course, \section_info $section) : bool {
    // CHANGED LINE ABOVE.
 ...
 * @return array
 */
function format_aimtopicb_reorder_sections(array $sections, $origins, \stdClass $target) : array {
    ...
    foreach ($originarray as $id => $section) {
        ...
        $sections[$id]->level = ($section->levelsan >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC
                                        && $section->levelsan != $originlevel) ?
                                    FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC       // Don't change topic level.
 ...
 * @return string
 */
function format_aimtopicb_image_attribution($imagename, $authorwithurl, $licencecode) : string {
    $o = '';
    ...
    $o .= \html_writer::tag('span', get_string('image', 'format_aimtopicb') . ": {$imagename}"
                            . (($authorhtml || $licencehtml) ? ',' : ''),
                            ['style' => 'white-space: nowrap;']) . ' ';
    if ($authorhtml) {
        $o .= \html_writer::tag('span', get_string('image_by', 'format_aimtopicb') . " {$authorhtml}"
                                . ($licencehtml ? ',' : ''),
                                ['style' => 'white-space: nowrap;']) . ' ';
    }
    if ($licencehtml) {
        $o .= \html_writer::tag('span', get_string('image_licence', 'format_aimtopicb') . " {$licencehtml}",
                                ['style' => 'white-space: nowrap;']);
    }
    return $o;
}
 ...
 * @return int|null
 */
function format_aimtopicb_duration_as_days($duration) {
    $days = null;
 ...
 */
function format_aimtopicb_week_date($date) {

    $config = get_config('format_aimtopicb');
