    ...
    if ($PAGE->user_allowed_editing()) {
        ...
        if (has_capability('moodle/course:sectionvisibility', $context)) {
            ...
            if ($hide && confirm_sesskey()) {
                format_aimtopicb_set_section_visible($course->id, $hide, 0);
                redirect($PAGE->url);
            }

            if ($show && confirm_sesskey()) {
                format_aimtopicb_set_section_visible($course->id, $show, 1);
                redirect($PAGE->url);
            }
            // END CHANGED.
        }
        ...
        if (!empty($section) && !empty($dest) &&
            ...
            try {                                                               // CHANGED: Use try/catch instead of return false.
                format_aimtopicb_move_section_to($course, $section, $destsection);
                if ($course->id == SITEID) {
                    redirect($CFG->wwwroot . '/?redirect=0');
                } else {
