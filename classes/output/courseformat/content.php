...
namespace format_aimtopicb\output\courseformat;
...
class content extends content_base {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $PAGE->requires->js_call_amd('format_aimtopicb/courseformat/courseeditor/mutations', 'init');
        ...
        for ($activesn = $displaysection; /* ... */
                ...
                $activesn = (($activesn->parentid
                    && ($activesn->levelsan > FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1
        ...
        $tabln = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1,
        ...
        $sectionatlevel = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,
                                     FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT, null);

        foreach ($sections as $thissection) {

            for ($level = $thissection->levelsan; $level < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC; $level++) {
            ...
            if ($thissection->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC && $showsection) {
                ...
                for ($level = max(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1, $thissection->levelsan); /* ... */
                     $level <= $thissection->pagedepthdirect
                                + ($format->show_editor()
                                    && $thissection->pagedepthdirect < FORMAT_AIMTOPICB_SECTION_LEVEL_PAGE_USE ? 1 : 0); /* ... */
                     $level++) {
                    ...
                    $newtab = new \tabobject("tab_id_{$thissection->id}_l{$level}", $url,
                        ...
                        $sectionname);
                    $newtab->level = $level - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT;
                    ...
                    // Add tab.
                    if ($level <= FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1) {
                        $tabs[] = $newtab;
                    } else {
            ...
            if ($thissection->nextanyid == $thissection->nextpageid
                ...
                for ($level = min($sectionatlevel[FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - 1]->pagedepthdirect + 1,
                                    FORMAT_AIMTOPICB_SECTION_LEVEL_PAGE_USE); /* ... */
                    ...
                    $url = new \moodle_url('/course/format/aimtopicb/_course_changenumsections.php', $params);
        ...
        $sectionnav = new \format_aimtopicb\output\courseformat\content\sectionnavigation($format, $displaysection);
    ...
    protected function export_sections(\renderer_base $output): array {
        ...
        $sectionatlevel = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,      // ADDED.
                                     FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT, null);
        ...
        foreach ($this->get_sections_to_display($modinfo) as $thissection) {
            ...
            $pageid = ($thissection->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) ? $thissection->id
