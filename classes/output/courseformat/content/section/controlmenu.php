...
namespace format_aimtopicb\output\courseformat\content\section;
...
class controlmenu extends controlmenu_base {
    ...
    public function section_control_items() {
        ...
        $onsectionpage = $section->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC; // ADDED.
        ...
        if ($section->section
            ...
            if ($onsectionpage) {                                               // CHANGED.
                if (has_capability('moodle/course:update', $coursecontext, $user)) {
                    $url = clone($baseurl);
                    if ($section->levelsan - 1 > FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT) { // Raise section. // CHANGED.
                        // CHANGED.
                        $url->param('sectionid', $section->id);
                        $url->param('destprevupid', $section->parentid);
                        $url->param('destlevel', $section->levelsan - 1);
                        $strmovelevelup = get_string_manager()->string_exists('move_level_up', 'format_aimtopicb') ?
                                            get_string('move_level_up', 'format_aimtopicb') : get_string('moveup');
                        // END CHANGED.
                    ...
                    $url = clone($baseurl);
                    if ($section->pagedepth + 1 <= FORMAT_AIMTOPICB_SECTION_LEVEL_PAGE_USE) { // Lower section. CHANGED.
                        // CHANGED.
                        $url->param('sectionid', $section->id);
                        $url->param('destparentid', $section->prevupid);
                        $url->param('destlevel', $section->levelsan + 1);
                        $strmoveleveldown = get_string_manager()->string_exists('move_level_down', 'format_aimtopicb') ?
                                            get_string('move_level_down', 'format_aimtopicb') : get_string('movedown');
                        // END CHANGED.
                ...
                if (isset($section->prevupid) && $section->prevupid != course_get_format($course)->fmtrootsectionid) {
                    ...
                    $strmovepageprev = get_string_manager()->string_exists('move_page_prev', 'format_aimtopicb') ?
                                        get_string('move_page_prev', 'format_aimtopicb') : get_string('moveleft');
                ...
                if (isset($section->nextupid)) { // Add a arrow to move section forward.
                    ...
                    $strmovepagenext = get_string_manager()->string_exists('move_page_next', 'format_aimtopicb') ?
                                        get_string('move_page_next', 'format_aimtopicb') : get_string('moveright');
                ...
                if ($section->prevpageid) { // Add a arrow to move section to previous page.
                    ...
                    $strmovetoprevpage = get_string_manager()->string_exists('move_to_prev_page', 'format_aimtopicb') ?
                                            get_string('move_to_prev_page', 'format_aimtopicb') : get_string('moveleft');
                ...
                if ($section->nextpageid) { // Add a arrow to move section to next page.
                    ...
                    $strmovetonextpage = get_string_manager()->string_exists('move_to_next_page', 'format_aimtopicb') ?
                                            get_string('move_to_next_page', 'format_aimtopicb') : get_string('moveright');
        ...
        if (array_key_exists('edit', $merged)) {
            $merged['edit']['url'] = new \moodle_url('/course/format/aimtopicb/_course_editsection.php',
        ...
        if (array_key_exists('delete', $merged)) {
            $url = new \moodle_url(
                '/course/format/aimtopicb/_course_editsection.php',
                [
                    'id' => $section->id,
