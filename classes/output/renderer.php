...
/**
 * Renderer for outputting the AIMtopicb course format.
 *
...
namespace format_aimtopicb\output;
...
/**
 * Basic renderer for AIMtopicb format.
 *
 * @copyright 2019 onwards James Calder and Otago Polytechnic
 * @copyright based on work by 2012 Dan Poltawski
 * @copyright based on work by 2020 Ferran Recio <ferran@moodle.com>
...
class renderer extends section_renderer {
    ...
    public function render(\renderable $widget) {
        ...
        if ($widget instanceof \templatable) {
            ...
            if (preg_match($specialrenderers, $fullpath, $matches)) {
                ...
                return $this->render_from_template('format_aimtopicb/courseformat/' . $matches['template'], $data);    // CHANGED.
            }
        }
    ...
    public function course_index_drawer(\core_courseformat\base $format): ?String {
        if ($format->uses_course_index()) {
            ...
            return $this->render_from_template('format_aimtopicb/courseformat/courseindex/drawer', []);
            // CHANGED LINE ABOVE.
        }
        return '';
    }
     ...
     *
     * @param \format_aimtopicb\courseheader $header header to render
     * @return string HTML to output.
     */
    protected function render_courseheader(\format_aimtopicb\courseheader $header) : string {
     ...
     *
     * @param \format_aimtopicb\coursecontentheaderfooter $headerfooter header/footer to render
     * @return string HTML to output.
     */
    protected function render_coursecontentheaderfooter(
                            \format_aimtopicb\coursecontentheaderfooter $headerfooter) : string {
