...
class global_navigation_wrapper {
    ...
    public function load_generic_course_sections(\stdClass $course, \navigation_node $coursenode) : array {
        ...
        $nodeln = array_fill(FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT,
                             FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC - FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1, $coursenode);
        ...
        foreach ($sections as $sectionid => $section) {
            ...
            if ($course->id == $SITE->id) {
                ...
                // ADDED.
                $firstlevel = max($section->levelsan, FORMAT_AIMTOPICB_SECTION_LEVEL_ROOT + 1);
                ...
                for ($level = $firstlevel; $level <= $lastlevel; $level++) {
                    ...
                    $sectionnode = $parentnode->add($sectionname, $url, \navigation_node::TYPE_SECTION,
                        null, $nodeid,
                        new \pix_icon($section->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC ? 'i/section'
                                                                                                 : 'e/bullet_list', ''));
                ...
                for ($level = $level; $level <= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC; $level++) {
