...
$PAGE->set_url('/course/format/aimtopicb/_course_editsection.php', array('id' => $id));
...
if ($deletesection) {
    ...
    if (course_can_delete_section($course, $sectioninfo)) {
        ...
        if ($confirm) {
            ...
            if ($sectioninfo->levelsan >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC) {
            ...
            $deleteurl = new \moodle_url('/course/format/aimtopicb/_course_editsection.php', $optionsyes);
...
if ($mform->is_cancelled()) {
    ...
    if (isset($data->level) && ($data->level != $sectioninfo->levelsan) && ($sectioninfo->section > 0)) {
        ...
        if (($sectioninfo->levelsan < FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC)
            && ($data->level >= FORMAT_AIMTOPICB_SECTION_LEVEL_TOPIC)) {
